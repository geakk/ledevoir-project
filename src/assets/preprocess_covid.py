...
start_date = pandas.to_datetime('2020-12-31 00:00:00', format='%Y-%m-%d %H:%M:%S')
end_date = pandas.to_datetime('2022-01-01 00:00:00', format='%Y-%m-%d %H:%M:%S')
mask = (data['Date de déclaration du cas'] > start_date) & (data['Date de déclaration du cas'] < end_date)
data = data.loc[mask]
