...
export class LineChartComponent implements OnChanges, AfterViewInit {
  ...
  public lineGroup: d3.Selection<SVGPathElement, unknown, null, undefined> | undefined;
  public lineGroupCovid:  d3.Selection<SVGPathElement, unknown, null, undefined> | undefined;
  private width = 800;
  private height = 700;
  private margin = 50;

  public svg: d3.Selection<SVGSVGElement, unknown, null, undefined> | undefined;
  public svgInner: d3.Selection<SVGGElement, unknown, null, undefined> | undefined;
 ...
 public data: any;
  ...
  private initializeChart(): void {
    ...
    let minDate = dates.reduce(function (a, b) { return a < b ? a : b; }); 
    let maxDate = dates.reduce(function (a, b) { return a > b ? a : b; });
    this.svg = d3
    ...
    this.xScale = d3.scaleTime().domain([minDate, maxDate]).range([this.margin, this.width - 2 * this.margin]);
  ...
  private drawChart(): void {
    ...
    const ordered = Object.keys(this.data).sort((a,b) => {return (new Date(b)).getTime() - (new Date(a)).getTime();}).reduce(
        (obj, key) => { 
    ...
    for (let i = 0; i< dates.length; i++){
        points.push([(this.xScale!(dates[i]) as number), (this.yScale!(articles[i]) as number)])
    }
    ...
    this.svgInner!.append("text")
        ...
        .text("Nombre d'articles portant sur la COVID/ Nombre de cas de COVID par jour");
    
    this.svgInner!.append("circle").attr("cx",200).attr("cy",130).attr("r", 6).style("fill", "red")
    this.svgInner!.append("circle").attr("cx",200).attr("cy",160).attr("r", 6).style("fill", "blue")
    this.svgInner!.append("text").attr("x", 220).attr("y", 130).text("Nombre d'articles").style("font-size", "15px").attr("alignment-baseline","middle")
    this.svgInner!.append("text").attr("x", 220).attr("y", 160).text("Cas de Covid").style("font-size", "15px").attr("alignment-baseline","middle")
  }
